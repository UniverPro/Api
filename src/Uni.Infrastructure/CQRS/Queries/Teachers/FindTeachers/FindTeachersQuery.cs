...
namespace Uni.Infrastructure.CQRS.Queries.Teachers.FindTeachers
{
    public class FindTeachersQuery : IQuery<IEnumerable<Teacher>>
    {
        ...
        public ISpecification<Teacher> ToSpecification()
        {
            ...
            {
                specification = specification.And(Spec<Teacher>.New(x => EF.Functions.Like(x.FirstName, $"%{FirstName}%")));
            }

            if (!string.IsNullOrEmpty(LastName))
            {
                specification = specification.And(Spec<Teacher>.New(x => EF.Functions.Like(x.LastName, $"%{LastName}%")));
            }
            
            if (!string.IsNullOrEmpty(MiddleName))
            {
                specification = specification.And(Spec<Teacher>.New(x => EF.Functions.Like(x.MiddleName, $"%{MiddleName}%")));
            }
